...
Upgrading an EXISTING agent is `domain-upgrade`'s job — it audits against the same
framework and recompiles onto the same spine; hand off to it instead of refitting here.

Both paths produce the same output set: `agents/<slug>.md`, `agents/<slug>-knowledge/{INDEX.md, README.md, <category>/<seed-stub>.md}`, `examples/<slug>-starter-prompts.yaml`. The KB scaffold (v0.6+) ships an indexable `INDEX.md` manifest plus per-category seed stubs so the agent is usable immediately after creation.
...
- User wants to **audit / uplevel / refit an EXISTING agent** → use `domain-upgrade`.
...
This skill only CREATES. If the user's request is actually about an EXISTING agent —
"refit X", "audit X", "uplevel X", "bring X up to spec" — stop and run `/domain-upgrade`
instead (it audits against the same 11 dimensions and recompiles onto the same spine).

Otherwise continue to **Phase 0.5 — Context Discovery & Synthesis** (below).
...
3. **On `go`, produce the file set** (do NOT write to disk yet):
   ...
   - Emit `name_ar:`, `categories:`, and `spine_version:` in frontmatter per
     ...
     read — it records which spine this agent was compiled against, so `domain-upgrade`
     can flag it when the spine advances.
...
| File | Destination |
|---|---|
| `<slug>.md` | `agents/<slug>.md` (new) |
| `<slug>-knowledge/INDEX.md` | `agents/<slug>-knowledge/INDEX.md` (new) |
| `<slug>-knowledge/README.md` | `agents/<slug>-knowledge/README.md` (new) |
| `<slug>-knowledge/<cat>/<stub>.md` | seed-stub per category that has a template under `templates/kb/<cat>/` |
| `<slug>-starter-prompts.yaml` | `examples/<slug>-starter-prompts.yaml` (new) |

(`domain-upgrade` writes the same file set against an existing agent — overwrite the
agent, create-if-missing for the KB, merge for prompts — using these same templates.)
...
- **Don't write code or run benchmarks.** This skill produces agent definitions, not evaluations.
